    ...
    try {
        ...
        */
        schemas.forEach(m => {
            ...
            typedi_1.default.set(m.name, schema);
        });
        repos.forEach(m => {
            let repoClass = require(m.path).default;
            let repoInstance = typedi_1.default.get(repoClass);
            typedi_1.default.set(m.name, repoInstance);
        });
        services.forEach(m => {
            let serviceClass = require(m.path).default;
            let serviceInstance = typedi_1.default.get(serviceClass);
            typedi_1.default.set(m.name, serviceInstance);
        });
        controllers.forEach(m => {
            // load the @Service() class by its path
            let controllerClass = require(m.path).default;
            // create/get the instance of the @Service() class
            let controllerInstance = typedi_1.default.get(controllerClass);
            // rename the instance inside the container
            typedi_1.default.set(m.name, controllerInstance);
        });
        return;
    }
    catch (e) {
